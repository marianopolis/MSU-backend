# functions for interaction with POSTGRESQL server, file hosting server and push notifications server
...
def get_posts(get_archived=False):
    ...
    if get_archived:
        archived_posts = Posts.query.filter_by(archived=True).order_by(desc(Posts.uploadtime)).all()            # getting all ordered archived posts
        return archived_posts
    else:
        unarchived_posts = Posts.query.filter_by(archived=False).order_by(desc(Posts.uploadtime)).all()         # getting all ordered unarchived posts
...
def get_documents():
    ...
    documents = Documents.query.order_by(desc(Documents.uploadtime)).all()                                      # getting all ordered documents
    # get document raw data in file hosting server from download link
    return documents
...
def get_forms():
    ...
    forms = Forms.query.order_by(desc(Forms.uploadtime)).all()                                                  # getting all ordered forms 
    return forms
